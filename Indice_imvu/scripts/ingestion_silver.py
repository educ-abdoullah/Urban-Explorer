...
print("DÉMARRAGE DE L'ÉTAPE SILVER (Nettoyage et Standardisation)...\n")

def trouver_fichier_recent(mot_cle):
    fichiers = [f for f in os.listdir(BRONZE_DIR) if mot_cle in f and f.endswith('.geojson')]
    if not fichiers:
        raise FileNotFoundError(f"Aucun fichier trouvé pour {mot_cle} dans {BRONZE_DIR}")
    return os.path.join(BRONZE_DIR, sorted(fichiers)[-1]) # Prend le plus récent

# TRAITEMENT DES ARBRES

print(" 1/4 - Traitement des arbres...")
fichier_arbres = trouver_fichier_recent("arbres")
arbres = gpd.read_file(fichier_arbres)
...
arbres.to_file(os.path.join(SILVER_DIR, "arbres_silver.geojson"), driver="GeoJSON")
print(f"   -> {len(arbres)} arbres traités et sauvegardés.")

# 2. TRAITEMENT DES ESPACES VERTS

print("2/4 - Traitement des espaces verts...")
fichier_ev = trouver_fichier_recent("espaces_verts")
parcs = gpd.read_file(fichier_ev)
...
parcs.to_file(os.path.join(SILVER_DIR, "espaces_verts_silver.geojson"), driver="GeoJSON")
...
iris.to_file(os.path.join(SILVER_DIR, "iris_silver.geojson"), driver="GeoJSON")
...
quartiers.to_file(os.path.join(SILVER_DIR, "quartiers_silver.geojson"), driver="GeoJSON")
...
arrondissements.to_file(os.path.join(SILVER_DIR, "arrondissements_silver.geojson"), driver="GeoJSON")
...
jardins = gpd.read_file(fichier_jardins)
...
jardins.to_file(os.path.join(SILVER_DIR, "jardins_partages_silver.geojson"), driver="GeoJSON")
print(f"   -> {len(jardins)} initiatives citoyennes traitées.")

print("\nÉTAPE SILVER TERMINÉE AVEC SUCCÈS.")
print(f"Vos données ultra-propres vous attendent dans le dossier : {SILVER_DIR}")